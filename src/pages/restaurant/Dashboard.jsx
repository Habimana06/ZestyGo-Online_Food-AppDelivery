...
function StatCard({ label, numericValue, icon, accent, bg, sparkData, prefix = '', suffix = '', delay = 0 }) {
  const animated = useCountUp(numericValue ?? 0);
  return (
    <div className="rsc" style={{ animationDelay: `${delay}ms` }}>
...
export default function RestaurantDashboard() {
  ...
  useEffect(() => {
    const run = async () => {
      setLoading(true);
      try {
        const [s, salesRows, statRows] = await Promise.all([
          api.restaurant.stats(),
          api.restaurant.reports.sales(`?preset=${period}`),
          api.restaurant.reports.orderStats(`?preset=${period}`),
        ]);
        setStats(s);
        setSales(Array.isArray(salesRows) ? salesRows : []);
        setOrderStats(Array.isArray(statRows) ? statRows : []);
      } catch (e) {
  ...
  const pendingCount     = Number(orderStats.find(r => r.status === 'pending')?.count || 0);
    ...
    return {
      ...
      datasets: [
        ...
        },
        {
          ...
          data: orders,
          borderColor: '#0ea5e9',
          backgroundColor: isBar ? 'rgba(14,165,233,0.8)' : 'rgba(14,165,233,0.10)',
          tension: 0.42, fill: !isBar,
          pointBackgroundColor: '#0ea5e9', pointRadius: isBar ? 0 : 4, pointHoverRadius: 7,
          borderWidth: isBar ? 0 : 2.5, borderRadius: isBar ? 6 : 0,
          yAxisID: 'y1',
        },
      ],
  ...
  const css = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; }
    ...
    .rst { font-family: 'DM Sans', sans-serif; background: #f8fafc; min-height: 100vh; }
    ...
    .rst-hdr h1 { font-family: 'Sora', sans-serif; font-size: 20px; font-weight: 800; color: #0f172a; letter-spacing: -0.5px; }
    .rst-hdr-sub { font-size: 12px; color: #94a3b8; margin-top: 3px; }
    .rst-hdr-right { display: flex; align-items: center; gap: 10px; }
    .rst-status-badge { background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 99px; padding: 6px 14px; font-size: 12px; color: #16a34a; font-weight: 600; display: flex; align-items: center; gap: 6px; }
    .rst-ico-btn { width: 36px; height: 36px; border-radius: 9px; border: 1px solid #e2e8f0; background: #fff; color: #64748b; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all .15s; font-size: 13px; }
    .rst-ico-btn:hover { background: #f1f5f9; color: #0f172a; }

    /* ── Body ── */
    .rst-body { padding: 24px 32px 48px; }
    ...
    .pending-banner-icon { width: 36px; height: 36px; border-radius: 10px; background: #fef3c7; display: flex; align-items: center; justify-content: center; color: #d97706; font-size: 16px; flex-shrink: 0; }
    .pending-banner-text { flex: 1; font-size: 13px; color: #92400e; font-weight: 500; }
    .pending-banner-count { font-family: 'Sora', sans-serif; font-size: 20px; font-weight: 800; color: #b45309; }

    /* ── Section label ── */
    .sec-lbl { font-size: 10px; font-weight: 700; letter-spacing: 1.4px; text-transform: uppercase; color: #94a3b8; margin-bottom: 12px; }

    /* ── Error ── */
    .rst-err { background: #fff; border: 1px solid #fecaca; border-radius: 14px; padding: 14px 18px; color: #dc2626; font-size: 13px; margin-bottom: 20px; display: flex; align-items: center; gap: 10px; }
    ...
    /* ── Stat cards ── */
    .rsc-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; margin-bottom: 24px; }
    @media(max-width:900px){ .rsc-grid { grid-template-columns: repeat(2,1fr); } }
    @media(max-width:600px){ .rsc-grid { grid-template-columns: 1fr; } }
    .rsc { background: #fff; border: 1px solid #e2e8f0; border-radius: 18px; padding: 20px; animation: fu .5s ease both; transition: transform .2s, box-shadow .2s; }
    .rsc:hover { transform: translateY(-4px); box-shadow: 0 16px 40px rgba(0,0,0,.08); }
    ...
    /* ── Tip card ── */
    .tip-card { background: #fff; border: 1px solid #e2e8f0; border-radius: 18px; padding: 20px 24px; display: flex; align-items: flex-start; gap: 14px; animation: fu .5s ease .2s both; }
    .tip-icon { width: 40px; height: 40px; border-radius: 11px; background: #fffbeb; color: #d97706; display: flex; align-items: center; justify-content: center; font-size: 17px; flex-shrink: 0; }
    .tip-title { font-family: 'Sora', sans-serif; font-size: 13px; font-weight: 700; color: #0f172a; margin-bottom: 4px; }
    .tip-body { font-size: 12px; color: #64748b; line-height: 1.6; }
  ...
  return (
    <>
      ...
      <div className="rst">
        ...
        {/* ── Header ── */}
        <header className="rst-hdr">
          <div>
            <h1>Restaurant Dashboard</h1>
            <div className="rst-hdr-sub">{greeting} 👋 &nbsp;·&nbsp; {today}</div>
          </div>
          <div className="rst-hdr-right">
            <div className="rst-status-badge">
              <span className="dot-green" style={{ margin: 0 }} />
              Restaurant Online
            </div>
            <button className="rst-ico-btn" title="Refresh" onClick={() => window.location.reload()}>
        ...
        <div className="rst-body">
          {error && <div className="rst-err"><i className="fas fa-exclamation-circle" />{error}</div>}

          {/* ── Pending orders alert ── */}
          {stats.pendingOrders > 0 && (
            <div className="pending-banner">
              <div className="pending-banner-icon"><i className="fas fa-bell" /></div>
              <div className="pending-banner-text">
                You have <strong>{stats.pendingOrders} pending order{stats.pendingOrders > 1 ? 's' : ''}</strong> waiting for your confirmation.
              </div>
              <div className="pending-banner-count">{stats.pendingOrders}</div>
            </div>
          )}
          ...
          <div className="h-strip">
            {[
              { lbl: 'Status',             val: <><span className="dot-green" />Online</> },
          ...
          <div className="rsc-grid">
            <StatCard label="Total Orders"   numericValue={Number(stats.totalOrders)}       icon="fa-receipt" accent="#2563eb" bg="#eff6ff" sparkData={ordersSparkData}  delay={0}   />
            <StatCard label="Pending Orders" numericValue={Number(stats.pendingOrders)}     icon="fa-clock"   accent="#d97706" bg="#fffbeb" sparkData={ordersSparkData}  delay={70}  />
            <StatCard label="Revenue (RWF)"  numericValue={Number(stats.revenue || 0)}      icon="fa-coins"   accent="#16a34a" bg="#f0fdf4" sparkData={revenueSparkData} prefix="RWF " delay={140} />
          </div>
          ...
          <div className="chart-row">
            ...
            <div className="card">
              <div className="card-hdr">
                <div>
                  <div className="card-title">
                    <i className="fas fa-circle-notch" style={{ color: '#0ea5e9', marginRight: 7 }} />
                    Orders by Status
                  </div>
          ...
          <div className="tip-card">
            <div className="tip-icon"><i className="fas fa-lightbulb" /></div>
            <div>
              <div className="tip-title">Pro Tip</div>
              <div className="tip-body">
                Keep your delivery fee accurate when accepting orders — your revenue charts use final delivered totals.
                A consistent menu and prompt confirmations help boost your completion rate.
              </div>
            </div>
          </div>

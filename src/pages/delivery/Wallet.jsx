...
/* Status config for withdrawal badges */
const STATUS_STYLE = {
  ...
  return (
    <button
      ...
      style={{
        ...
        borderColor: disabled ? '#e2e8f0' : '#d1fae5',
...
export default function DeliveryWallet() {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      const data = await api.delivery.wallet();
  ...
  /* Sparkline: withdrawal amounts over time (last 7) */
  ...
  const css = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; }
    ...
    .wlt-withdraw-btn {
      ...
      background: #10b981; color: #fff; font-family: 'Sora', sans-serif;
      font-size: 13px; font-weight: 700; padding: 9px 20px; border-radius: 11px;
      border: none; cursor: pointer; transition: all .15s;
    }
    .wlt-withdraw-btn:hover:not(:disabled) { background: #059669; transform: translateY(-1px); box-shadow: 0 6px 20px rgba(16,185,129,.3); }
    ...
    /* ── Two-col layout: card + side panel ── */
    ...
    .balance-card {
      ...
      box-shadow: 0 24px 60px rgba(5,150,105,.35), 0 8px 20px rgba(0,0,0,.2);
      transition: transform .3s ease, box-shadow .3s ease;
      cursor: default;
    }
    .balance-card:hover {
      transform: translateY(-4px) rotateX(2deg);
      box-shadow: 0 32px 72px rgba(5,150,105,.4), 0 12px 28px rgba(0,0,0,.22);
    }
    ...
    .bc-dots {
      ...
      display: flex; justify-content: flex-end; padding: 0 36px;
    ...
    .bc-chip {
      ...
      position: relative; margin-bottom: 28px; z-index: 1;
      box-shadow: 0 2px 8px rgba(0,0,0,.3);
    }
    .bc-chip::before {
      content: ''; position: absolute; inset: 0;
      border-radius: 7px;
      background: repeating-linear-gradient(0deg, transparent, transparent 8px, rgba(0,0,0,.08) 8px, rgba(0,0,0,.08) 9px),
                  repeating-linear-gradient(90deg, transparent, transparent 10px, rgba(0,0,0,.08) 10px, rgba(0,0,0,.08) 11px);
    }
    .bc-chip::after {
    ...
    .bc-status-dot { width: 6px; height: 6px; border-radius: 50%; background: #6ee7b7; box-shadow: 0 0 0 3px rgba(110,231,183,.2); animation: pulse 2s infinite; }
    ...
    .bc-withdraw-btn {
      ...
      padding: 9px 18px; border-radius: 10px; cursor: pointer; transition: all .2s;
    ...
    .filter-tab { font-size: 11px; font-weight: 600; padding: '4px 10px'; border-radius: 7px; border: none; cursor: pointer; transition: all .15s; }

    /* ── Withdrawal rows ── */
    .wr-row {
      display: flex; align-items: center; gap: 12px; padding: 12px 0;
      border-bottom: 1px solid #f8fafc; animation: fu .4s ease both;
    ...
    .wlt-input:focus { border-color: #10b981; box-shadow: 0 0 0 3px rgba(16,185,129,.1); }
    .wlt-select {
      width: 100%; padding: 12px 16px; border: 1.5px solid #e2e8f0; border-radius: 12px;
      font-family: 'DM Sans', sans-serif; font-size: 14px; color: #0f172a;
      background: #fff; outline: none; transition: border-color .15s; cursor: pointer;
    }
    .wlt-select:focus { border-color: #10b981; box-shadow: 0 0 0 3px rgba(16,185,129,.1); }
    ...
    .method-opt {
      flex: 1; padding: 12px 8px; border-radius: 12px; border: 1.5px solid;
      cursor: pointer; text-align: center; transition: all .15s; background: #fff;
    }
    .modal-cancel-btn {
      padding: 9px 20px; border-radius: 11px; border: 1.5px solid #e2e8f0;
    ...
    .modal-submit-btn:hover:not(:disabled) { background: #059669; }
    ...
    <>
              ...
              <div className="balance-card">
                ...
                <div className="bc-top">
                  <div className="bc-logo">
                    <i className="fas fa-motorcycle" style={{ marginRight: 6, opacity: .7 }} />
                    Delivery Wallet
                  </div>
            ...
            <div className="side-panel">
              ...
              <div className="quick-actions">
                ...
                <div className="qa-grid">
                  {[
                    { icon: 'fa-arrow-up',     bg: '#f0fdf4', color: '#16a34a', lbl: 'Withdraw',      action: () => setOpen(true), disabled: balance <= 0 },
              ...
              <div className="earnings-card">
                ...
                {(() => {
                  ...
                  const rows = [
                    { lbl: 'Available', val: `RWF ${balance.toLocaleString()}`,          pct: Math.round(balance / total * 100),          color: '#10b981', bg: '#f0fdf4', icon: 'fa-wallet'      },
                    ...
                    { lbl: 'Pending',   val: `${pendingCount} req`,                      pct: pendingCount > 0 ? 30 : 0,                 color: '#d97706', bg: '#fffbeb', icon: 'fa-clock'         },
                  ];
                  return rows.map(r => (
              ...
              <div style={{
                ...
                border: '1px solid #a7f3d0', borderRadius: 16, padding: '14px 18px',
                ...
                <div>
                  <div style={{ fontFamily: 'Sora,sans-serif', fontSize: 12, fontWeight: 700, color: '#065f46', marginBottom: 3 }}>Pro Tip</div>
                  <div style={{ fontSize: 11, color: '#047857', lineHeight: 1.55 }}>
                    Your balance grows automatically after each delivered order is confirmed. Withdraw anytime — requests are processed within 24 hours.
                  </div>
                </div>
              </div>
          ...
          <div className="hist-card">
            <div className="hist-hdr">
              <div className="hist-title">
                <i className="fas fa-history" style={{ color: '#10b981', marginRight: 7 }} />
                All Requests
              </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10 }}>
            ...
            >
              {submitting
                ...
                : <><i className="fas fa-arrow-up" />Submit request</>
              }
            </button>
          </div>
        }
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          {/* Info */}
          <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 12, padding: '12px 16px', fontSize: 13, color: '#166534', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 6 }}>
              <span style={{ fontSize: 12, color: '#94a3b8' }}>
                Available: <strong style={{ color: '#16a34a' }}>{money(balance)}</strong>
              </span>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {quickAmounts.map(v => (
                  <QuickAmt
                    ...
                    value={v}
                    balance={balance}
                    onClick={v => setForm(f => ({ ...f, amount: String(v) }))}
                  />
                ))}
              </div>
            </div>
